...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Open Code Neo update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Open Code Neo", () => {
  expect(
  ...
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.opencode-neo/bin/opencode",
          version: "1.16.2",
...
test("ignores stale background Open Code Neo checks after removing a WSL server", async () => {
  persistedServers = []
...
test("ignores stale startup Open Code Neo checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveOpencode: async () => {
      ...
      return "/home/me/.opencode-neo/bin/opencode"
    },
  }
}

...
const originalEnv = {
  OPENCODE_AUTH_CONTENT: process.env.OPENCODE_NEO_AUTH_CONTENT,
  OPENCODE_EXPERIMENTAL_WORKSPACES: process.env.OPENCODE_NEO_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENCODE_NEO_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })

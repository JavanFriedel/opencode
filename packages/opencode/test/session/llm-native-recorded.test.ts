...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENCODE_NEO_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  ...
  },
  {
    ...
    canRecord: () => Boolean(process.env.OPENCODE_NEO_RECORD_CONSOLE_TOKEN && process.env.OPENCODE_NEO_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        ...
        api: zenURL(process.env.OPENCODE_NEO_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENCODE_NEO_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENCODE_NEO_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {

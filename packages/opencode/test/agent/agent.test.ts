...
it.instance("plan agent denies edits except .opencode-neo/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".opencode-neo/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".opencode-neo", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENCODE_NEO_TEST_HOME
      process.env.OPENCODE_NEO_TEST_HOME = test.directory

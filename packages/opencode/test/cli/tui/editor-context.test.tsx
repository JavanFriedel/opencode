...
const originalOpencodePort = process.env.OPENCODE_NEO_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.OPENCODE_NEO_EDITOR_SSE_PORT = originalOpencodePort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.OPENCODE_NEO_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.OPENCODE_NEO_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with OPENCODE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.OPENCODE_NEO_EDITOR_SSE_PORT = "4020"

...
export function alreadyInstalled() {
  return process.env["OPENCODE_NEO_CALLER"] === "vscode" || process.env["OPENCODE_NEO_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-devopencode-neo"], {
    nothrow: true,
  })
